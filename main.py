...
def predict_disease(request: SymptomsRequest):
    ...
    if invalid_symptoms:
        raise HTTPException(status_code=400, detail=f"Invalid symptoms: {', '.join(invalid_symptoms)}")
    ...
    return {
        ...
        "description": desc.tolist() if len(desc) > 0 else "No description available",
        "precautions": pre,
        "medications": med,
        "diet": die,
        "workout": wrkout
